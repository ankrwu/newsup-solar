...
class ChinaPVCrawler(BaseCrawler):
    """中国光伏行业协会爬虫 - 行业协会官方媒体"""
    ...
    async def fetch_article_urls(self) -> List[str]:
        ...
        logger.info(f"从中国光伏行业协会总共找到 {len(article_urls)} 篇文章")
    ...
    def _extract_title(self, soup: BeautifulSoup) -> str:
        ...
        for selector in selectors:
            ...
            if elem:
                ...
                title = re.sub(r'[-_]\s*中国光伏行业协会.*$', '', title)
                title = re.sub(r'[-_]\s*chinapv.*$', '', title, flags=re.IGNORECASE)
                return title
